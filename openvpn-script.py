...
def iptables(ar, ip, net):
	os.spawnv(os.P_WAIT, "/sbin/iptables", ["iptables", "-%s"%ar.upper(), "FORWARD", "-s", ip, "-d", net, "-j", "ACCEPT"])
	os.spawnv(os.P_WAIT, "/sbin/iptables", ["iptables", "-%s"%ar.upper(), "FORWARD", "-s", net, "-d", ip, "-j", "ACCEPT"])
	...
	if networks:
		...
		for network in networks:
			try:
				...
				netmask = socket.inet_ntoa(struct.pack(">I", (0xffffffff << (32 - int(cidr))) & 0xffffffff))
				c_conf.write('push "route %s %s"\n'%(net,netmask))
				iptables("a", os.environ['ifconfig_pool_remote_ip'], network[0])
			except ValueError:
				continue
	sys.exit(0)
...
def client_disconnect():
	for network in c.execute('SELECT network FROM network_map WHERE username = ?', (os.environ['username'],)):
		iptables("d", os.environ['ifconfig_pool_remote_ip'], network[0])
	sys.exit(0)
